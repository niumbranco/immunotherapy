...
taxa_file <- "~/Documents/EIDD/2A/STAGE_2A/DOCUMENTS_DE_STAGE/immunotherapy/PRJEB61942/reads_taxonomy/taxa.rds"
...
#transformer en dataframe pour ggplot
phylum_df <- psmelt(ps.phylum) 

#nettoyer et ordonner les phylums les plus abondants
top_phyla <- phylum_df %>%
  group_by(Phylum) %>%
...
phylum_df$Phylum[!(phylum_df$Phylum %in% top_phyla)] <- "Other"
...
p_phylum <- ggplot(phylum_df, aes(x = Sample, y = Abundance, fill = Phylum)) +
  ...
  labs(title = "Top 10 phyla - relative abundance", y = "Abundance", x = NULL) +
  theme_classic() +
...
ggsave("diversity_combined_plot.png", combined_plot, width = 10, height = 8, bg = "white")
