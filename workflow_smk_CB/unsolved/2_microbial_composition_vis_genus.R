...
relabs_in <- file.path(work_dir, paste0("tables/Relabs_G.tsv"))
...
relabs_tb <- relabs_tb %>%
  mutate(name = ifelse(value < 1, "others", name))
...
plot_M <- plots_list(relabs_tb, "M") +
  ...
  labs(title = "Microbial composition: genus")
...
pdf(file = file.path(work_dir, "visuals", "0_microbial_composition_conditions_genus.pdf"),
    height = 20/2.54,
    width = 30/2.54)
print(comb_plot)
dev.off()
